...
import com.zeroton.reservation.reservation_zeroton.email.util.CookieUtil;
...
import com.zeroton.reservation.reservation_zeroton.member.dto.VerifyEmailDto;
...
import com.zeroton.reservation.reservation_zeroton.member.dto.SignInForm;
import com.zeroton.reservation.reservation_zeroton.member.dto.SignUpForm;
...
public class AuthService {
    ...
    public Member signUpMember(SignUpForm signUpForm) throws IllegalStateException, IllegalArgumentException {
        ...
        if(!isDkuStudent(signUpForm.getEmail())){
            throw new IllegalArgumentException("단국대학교 이메일(dankook.ac.kr)만 가입이 가능합니다.");
        }
    ...
    private boolean isDkuStudent(String email) {
        String emailAddress = email.substring(email.indexOf("@") + 1);
        return emailAddress.equals("dankook.ac.kr");
    }

    public Member loginUser(SignInForm signInForm) throws NotFoundException{
        if(!isDkuStudent(signInForm.getEmail())) throw new IllegalArgumentException("단국대학교 학생이 아닙니다.");
    ...
    public void sendVerificationMail(String email) throws NotFoundException {
        ...
        emailService.sendMail(email, "[단국대학교 소프트웨어학과 APP] 회원가입 인증메일입니다.", content);
        log.info(content);
    }
    ...
    public void sendPasswordChangeVerificationMail(String email) throws NotFoundException {
        ...
        String content = "다음의 인증 번호를 입력해 인증을 완료해주세요. \n" + authentication ;
        redisUtil.setDataExpire(authentication+"",email, 60 * 3L);
        emailService.sendMail(email, "[단국대학교 소프트웨어학과 APP] 비밀번호 변경 인증메일입니다.", content);
        log.info(content);
    }
